...
### Pioneer CDJ-1000 MK1 → ESP32-S3 USB-MIDI controller for Traktor Pro 4

Resurrect a Pioneer CDJ-1000 MK1 (2001, no native USB/MIDI) as a class-compliant USB-MIDI / HID controller for Traktor Pro 4. The OEM mainboard and CD drive are removed; the chassis, jog wheel assembly, pitch fader, button PCBs, pots, and encoders are kept and re-wired to an **ESP32-S3 DevKitC-1 (N16R8)** that speaks USB-MIDI natively via TinyUSB.

> **Status:** architecture v0.1 — wiring map locked, GPIO assignments tentative, firmware not yet started.
...
| Jog optical encoder (135 frames/rev) | CD drive + servo board |
| Jog touch sheet | OEM display board |
| 100 mm pitch fader | OEM VFD (optional re-use TBD) |
| Button PCBs (hot cues, loop, search, tempo, etc.) | Power supply (USB-powered now) |
...
| Buttons (16+) | Button PCBs | 2× 74HC4067, S0–S3 = GPIO 8/9/10/11, SIG = GPIO 12/+1 | |
...
| Lee Smith / DJLegionUK — CDJ-1000 MK1 Teensy build | Original MK1 hack. [forum.djtechtools.com/showthread.php?t=94870](https://forum.djtechtools.com/showthread.php?t=94870) / [djtechtools.com/2017/06/28/hacking-cdj-1000mk1...](https://djtechtools.com/2017/06/28/hacking-cdj-1000mk1-work-midi-controller-traktor-scratch/) |
| Pioneer service manual | [instrumentalparts.com/content/pdf/CDJ1000MK1.pdf](https://instrumentalparts.com/content/pdf/CDJ1000MK1.pdf) — PCB layouts, connector pinouts, mechanical drawings. |
...
1. Confirm jog encoder voltage + PPR from service manual
2. Jog-touch sensor type — capacitive vs pressure sheet (determines S3 native touch vs comparator front-end)
3. Trace button PCB connector (CN) pinout → 4067 channel map
4. Lock final GPIO map after button count (may need a 3rd mux)
5. Display decision — jog-center IPS vs simple status OLED vs no display
6. Firmware path — ESP-IDF + TinyUSB MIDI **vs** Arduino-ESP32 + Adafruit TinyUSB
